...
websocket_init(State) ->
    io:format("New connection~n"), {ok, State}.

websocket_handle({text, Msg}, State) ->
    Data = jsx:decode(Msg, []),
    case Data of
      [{<<"name">>, Name}] ->
	  cowboy_server:enter(self(), Name), {ok, State};
      [{<<"type">>, <<"User disconected">>}] ->
	  io:format("Disconect~n"),
	  cowboy_server:send_message(self(),
				     <<"User disconected">>),
	  ...
	  {ok, State};
      [{<<"names">>, _}] ->
	  cowboy_server:send_names(), {ok, State};
      _Any -> {ok, State}
    end;
